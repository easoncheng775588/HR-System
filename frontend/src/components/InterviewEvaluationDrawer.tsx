...
import { Button, Descriptions, Drawer, Form, Input, Select, Space, message } from 'antd'
...
const InterviewEvaluationDrawer: React.FC<InterviewEvaluationDrawerProps> = ({
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      entryLevelSuggestion: record?.entryLevelSuggestion || record?.appliedLevel || '',
      score: record?.score || '',
    ...
    const loadDetail = async () => {
      ...
      try {
        ...
        if (res.data?.returnCode === 'SUC0000' && res.data?.body) {
          ...
          form.setFieldsValue({
            entryLevelSuggestion: body.entryLevelSuggestion || '',
            score: body.score || '',
            hireSuggestion: body.hireSuggestion || '',
          })
        }
      } catch (_error) {
  ...
  const submit = async () => {
    try {
      ...
      const payload = {
        ...
        interviewDate: String(record?.confirmedInterviewTime || ''),
  ...
  return (
    ...
    >
      <Descriptions column={1} size="small">
        ...
        <Descriptions.Item label="面试日期">{formatDateTimeText(String(record?.confirmedInterviewTime || ''))}</Descriptions.Item>
      </Descriptions>

      <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
        <Form.Item name="entryLevelSuggestion" label="入场职级建议" rules={[{ required: true, message: '请输入入场职级建议' }]}>
          <Input />
        </Form.Item>
